...
from bot.ai_services import call_local_ai_json, generate_empenho_text
...
async def process_auditor_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_pdf:
            ...
            if not text:
                await msg.edit_text(
                    "⚠️ Não foi possível extrair texto do PDF (imagem escaneada?). Envie apenas PDFs em formato texto no momento."
                )
                context.user_data.clear()
                return
        else:
...
---"""
        ...
        # Call local AI with automatic JSON parsing and retry
        ...
        score_emoji = (
            "🟢"
            if score < 30
            else ("🟡" if score < 60 else ("🟠" if score < 80 else "🔴"))
        )
