    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "📅 Calendário de Pagamentos", callback_data="cmd_calendario"
            ),
        ],
        [InlineKeyboardButton("— 📝 PROTOCOLOS E PROCESSOS —", callback_data="ignore")],
        [
            InlineKeyboardButton(
                "🔎 Buscar Protocolo/Ofício", callback_data="cmd_buscar_protocolos"
            ),
        ],
        [
        ...
        [
            InlineKeyboardButton(
                "🛒 Nova Aquisição", callback_data="cmd_nova_aquisicao"
            ),
            InlineKeyboardButton("✈️ Calc. Diárias", callback_data="cmd_calc_diarias"),
        ],
        [
            InlineKeyboardButton(
                "🏢 Consultar CNPJ", callback_data="cmd_consulta_cnpj"
            ),
        ...
        [
            ...
            InlineKeyboardButton(
                "✨ Renomear Arq.", callback_data="cmd_renomear_arquivo"
            ),
        ],
        [
            InlineKeyboardButton("🔍 Auditor NF", callback_data="cmd_auditor_nf"),
            InlineKeyboardButton(
                "🏦 Extrato Bancário", callback_data="cmd_extrato_bancario"
            ),
        ],
        [
        ...
        [InlineKeyboardButton("— 📤 PDF EMPENHO —", callback_data="ignore")],
        [
            InlineKeyboardButton(
                "📤 Solicitar PDF Empenho", callback_data="cmd_pdf_empenho"
            ),
        ],
    ...
    return InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton(
                    "⚡ Em Progresso", callback_data="status_in-progress"
                ),
            ],
            [
...
def keyboard_financeiro(ano: int, mes: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "⬅ Mês Anterior", callback_data=f"fin_{ano_ant}_{mes_ant}"
                ),
                InlineKeyboardButton(
                    "➡ Próximo Mês", callback_data=f"fin_{ano_prox}_{mes_prox}"
                ),
            ],
            [
                InlineKeyboardButton(
                    "📋 Empenhados", callback_data=f"fin_emp_{ano}_{mes}"
                ),
                InlineKeyboardButton(
                    "⏳ Pendentes", callback_data=f"fin_pend_{ano}_{mes}"
                ),
            ],
...
def keyboard_credores_pagina(
    credores: list, pagina: int, total_paginas: int
) -> InlineKeyboardMarkup:
    kb = []
    for c in credores:
        nome_curto = (c["nome"][:25] + "…") if len(c["nome"]) > 25 else c["nome"]
        kb.append(
            [
                InlineKeyboardButton(
                    f"🏢 {nome_curto}", callback_data=f"pdf_credor_{c['id']}"
                )
            ]
        )
    nav = []
    if pagina > 1:
        nav.append(
            InlineKeyboardButton(
                "⬅ Anterior", callback_data=f"pdf_credores_pg_{pagina - 1}"
            )
        )
    if pagina < total_paginas:
        nav.append(
            InlineKeyboardButton(
                "Próxima ➡", callback_data=f"pdf_credores_pg_{pagina + 1}"
            )
        )
    if nav:
        kb.append(nav)
    kb.append([InlineKeyboardButton("🔙 Voltar", callback_data="cmd_menu")])
    return InlineKeyboardMarkup(kb)


def keyboard_mes_ano(credor_id: int, ano: int, mes: int) -> InlineKeyboardMarkup:
    mes_ant = mes - 1 if mes > 1 else 12
    ano_ant = ano if mes > 1 else ano - 1
    mes_prox = mes + 1 if mes < 12 else 1
    ano_prox = ano if mes < 12 else ano + 1
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "⬅ Mês Anterior",
                    callback_data=f"pdf_mes_{credor_id}_{ano_ant}_{mes_ant}",
                ),
                InlineKeyboardButton(
                    "➡ Próximo Mês",
                    callback_data=f"pdf_mes_{credor_id}_{ano_prox}_{mes_prox}",
                ),
            ],
            [
                InlineKeyboardButton(
                    "📤 GERAR E ENVIAR PDF",
                    callback_data=f"pdf_gerar_{credor_id}_{ano}_{mes}",
                )
            ],
            [
                InlineKeyboardButton(
                    "🔙 Voltar aos credores", callback_data="pdf_credores_pg_1"
                )
            ],
        ]
    )
