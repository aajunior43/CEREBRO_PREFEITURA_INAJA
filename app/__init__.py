...
def _register_hooks(app):
    ...
    def mark_request_start():
        ...
        g._request_path = request.path if request else ''
        g._request_full_path = request.full_path.rstrip('?') if request else ''
        g._request_ip = request.headers.get('X-Forwarded-For', request.remote_addr or '-').split(',')[0].strip() if request else '-'
    ...
    def compress_response(response):
        ...
        if started_at is not None:
            ...
            if request.path.startswith('/api/') and elapsed_ms >= 250:
                app.logger.warning('Slow request %.1fms %s %s', elapsed_ms, request.method, request.path, response.status_code)
        
        # Compressão gzip para APIs
