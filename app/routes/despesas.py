"""
app/routes/despesas.py — CSV expense imports (migrated from routes/all_routes.py)
"""
import json
...
def despesas_resumo(imp_id):
    ...
    val_cols = [
        c
        for c in colunas
        if any(k in c.lower() for k in ["saldo", "valor", "empenhado", "liquidado", "pago"])
    ]
    ...
    natureza_col = next(
        (c for c in colunas if "natureza" in c.lower() and "descrição" not in c.lower() and "descricao" not in c.lower()),
        None,
    )
    recurso_col = next(
        (c for c in colunas if "recurso" in c.lower() and "descrição" not in c.lower()), None,
    )
...
def despesas_ia():
    ...
    system_prompts = {
        ...
        "cortes": f"Analista de eficiência. Hoje é {today}. Identifique contenção de gastos. Português do Brasil.",
    }
    if action not in system_prompts:
        return jsonify({"error": f"Ação inválida. Use: {', '.join(system_prompts.keys())}"}), 400
    messages = [
...
def empenhos_csv_importar():
    try:
        ...
        now = _dt_now.strftime("%Y-%m-%d %H:%M:%S")
        conn = get_db()
        cur = conn.cursor()
        cur.execute(
