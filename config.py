...
class Settings:
    ...
    debug: bool = os.environ.get("APP_DEBUG", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    }
    reloader: bool = os.environ.get("APP_RELOADER", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    openrouter_backoff_base: float = float(
        os.environ.get("OPENROUTER_BACKOFF_BASE", "1.5")
    )
    openrouter_cache_ttl_seconds: int = int(
        os.environ.get("OPENROUTER_CACHE_TTL_SECONDS", "900")
    )
